...
function Weapons() {
    ...
    const fetchWeapons = async () => {
      try {
        const response = await axios.get(
          `https://api.open5e.com/weapons?page=${page}`
        );
        setWeapons((prevItems) => [...prevItems, ...response.data.results]);
      } catch (error) {
  ...
  const handleSearch = async (event) => {
    ...
    try {
      const response = await axios.get(
        `https://api.open5e.com/weapons?page=1&search=${query}`
      );
  ...
  return (
    ...
    >
      <h1 style={{ textAlign: "center" }}>Weapons List</h1>
      <TextField
      ...
      >
        {Weapons.map((item, index) => (
          ...
          >
            <Link to={`/weapons/${item.slug}`}>{item.name}</Link>
          </div>
        ))}
      </div>
      {page < 3 && (
